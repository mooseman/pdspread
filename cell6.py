...
def draw_cell(): 
   # Column headings 
   for x,y in zip(range(6, 75, 7), range(65, 75)): 
      cell = curses.newwin(3, 7, 1, x) 
      cell.addstr(1, 3, str(chr(y)), curses.A_BOLD)
      cell.box() 
      cell.refresh()     
    
   # Row headings 
   for x,y in zip(range(3, 22, 3), range(1, 11)): 
      cell = curses.newwin(3, 5, x, 0) 
      cell.addstr(1, 2, str(y), curses.A_BOLD)
      cell.box() 
      cell.refresh()        
