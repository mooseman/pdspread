...
class sheet(highlight, storage): 
   def __init__(self, scr):       
      ...
      # Setting the first parameter to 3 means that our column 
      # headings do not scroll off the screen. 
      self.scr.setscrreg(3, self.maxrows-1)               
      ...
      self.rowheadposlist = list((y,x) 
         for y in range(2, self.maxrows-1) 
            for x in range(2, 3) )  
      self.rowheaddata = list(range(1, self.maxrows-1) ) 
      
      self.colheadposlist = list( (y,x) 
         for y in range(1, 2) 
            for x in range(10, self.maxcols-1, 7) )  
